<section id="call-footer">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h2 class="section-title mt-3">Receba novidades sobre o evento!</h2>         
            </div>
        </div>

        <div class="row justify-content-center">            
            <div class="col-lg-4 col-12">
                <h3 class="call-footer-text">Receba novidades sobre o evento!</h3>
            </div>
            <div class="col-lg-7 col-12">
                <form>
                    <div class="form-row align-items-center">
                        <div class="form-group col-lg-8 col-12">
                            <input type="text" class="form-control input-lg mb-3" id="inputName" name="inputName" aria-describedby="nameHelp" placeholder="Digite seu Nome" />
                            <input type="email" class="form-control input-lg mb-4" id="inputEmail" name="inputEmail" aria-describedby="emailHelp" placeholder="Digite seu Email" />
                        </div>
                        <div class="form-group col-lg-4 col-12">
                            <button type="submit" class="btn btn-dark">Quero receber!</button>  
                        </div>
                    </div>                    
                </form>
            </div>
        </div>

    </div>        
</section>